...
public class Heap<T extends Comparable<T>> {
    ...
    private void heapifyUp() {
        // TODO
    	int childIndex = container.size() - 1;
    	int parentIndex = (childIndex - 1)/2;
    	T child = container.get(childIndex);
    	T parent = container.get(parentIndex);
    	

    	//child < parent and there is still parent
    	while(parent.compareTo(child) > 0 && hasParent(childIndex)) {
    		container.set(parentIndex,child);
    		container.set(childIndex, parent);
    		childIndex = parentIndex;
    		parentIndex = (childIndex - 1)/2;
    		child = container.get(childIndex);
    		parent = container.get(parentIndex);
    	}
